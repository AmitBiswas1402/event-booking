...
const FeatSports = ({ events }: { events: HomeEvent[] }) => {
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-white/15 bg-[#16161d] p-10 text-center">
        <Trophy className="size-7 text-slate-500" />
        <p className="text-sm font-semibold">No sports events yet</p>
        <p className="text-xs text-slate-400">Sports events will appear here once organizers publish them.</p>
      </div>
    );
  }
      ...
      {events.map((event) => (
        ...
        >
          {event.bannerUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-emerald-600/30 to-teal-600/30">
              <Trophy className="size-10 text-white/40" />
            </div>
          )}
